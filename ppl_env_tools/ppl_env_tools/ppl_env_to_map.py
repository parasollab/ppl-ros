...
def env_to_png(directory,env_file,png_file):

    f = open(directory + '/' + env_file,'r')
    lines = f.readlines()
...
def convert_to_binary(directory,png_file, binary_file):
    ...
    # Save image
    cv2.imwrite(directory + '/' + binary_file, bw_img)

def make_ros_map(directory,file_name):
...
if __name__ == '__main__':
    ...
    env_to_png(directory,env_file,png_file)
    convert_to_binary(directory,png_file,binary_file)
    make_ros_map(directory,binary_file)
